...
function list(req, res) {
  const db = getDb();
  const users = db.prepare('SELECT id, username, email, role, pole, created_at FROM users ORDER BY username').all();
  res.json(users);
}
...
function create(req, res) {
  ...
  const { username, email, password, role, pole } = result.data;
  const db = getDb();
  ...
  const info = db.prepare(`
    INSERT INTO users (username, email, password, role, pole) VALUES (?, ?, ?, ?, ?)
  `).run(username, email, hash, role, pole || null);

  const user = db.prepare('SELECT id, username, email, role, pole, created_at FROM users WHERE id = ?').get(info.lastInsertRowid);
...
function update(req, res) {
  ...
  const updated = db.prepare('SELECT id, username, email, role, pole, created_at FROM users WHERE id = ?').get(req.params.id);
  res.json(updated);
}
