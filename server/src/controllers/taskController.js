...
const taskSchema = z.object({
  ...
  duration_hours: z.number().int().min(0).max(23).default(0),
...
function create(req, res) {
  ...
  const info = db.prepare(`
    INSERT INTO tasks (project_id, title, duration_days, duration_hours, status, position, start_date, due_date, depends_on, assigned_to, notes, earliest_start, latest_end)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(req.params.id, title, duration_days, duration_hours ?? 0, status, position,
...
function update(req, res) {
  ...
  if (duration_hours  !== undefined) { changes.push('duration_hours = ?');  params.push(duration_hours); }
